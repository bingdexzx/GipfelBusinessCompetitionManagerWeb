<template>
  <div class="products-manager">
    ...
    >
      <template #parts="{ row }">
        <span v-if="!row._parts?.length && !row.productParts?.length" style="color: #c0c4cc">-</span>
        <el-tag
          v-for="pp in row.productParts || row._parts || []"
          :key="pp.id || pp.partId"
          size="small"
...
async function loadPartOptions() {
  try {
    // 必须带 competitionId：超管不受 apply_competition_scope 过滤，漏带会把其他比赛的零件
    // 混进下拉并被写成本比赛的配比关联（审计 W-03）
    if (!compStore.competitionId) {
      partOptions.value = [];
      return;
    }
    const res: any = await api.get("/parts", {
