          ...
          <template v-else-if="selectedNode.type === 'input'">
            ...
            <div class="ge-node-desc">
              ...
              <div v-else-if="selectedNode.data.type === 'partList'" class="ge-tip">
                ...
                <br />
                本输入源有五个输出端点：<b>输出</b>（原始字典，可接入公式作用域）、
                <b>所需原料</b>（按比赛查询每个零件的配比，将「数量 × 系数」按原料累加，输出
                <code>{"原料名称": 总数量}</code> 字典，连线到下游字典/公式端口即可直接拿到该批零件所需的全部原料及数量，无需手动拆解）、
                <b>所需的科技节点</b>（按比赛查询清单中每个零件的「科技需求(TechNode)」，收集这些科技节点名称，去重后输出字符串数组，连线到下游列表/字典端口即可直接拿到该批零件所需的全部科技节点，无需手动查表）、
                <b>零件总件数</b>（直接把清单中各零件数量相加，连线到下游数值端口即可参与效果/检查计算，无需查表）<b>与</b>
                <b>所需原料总数量</b>（按比赛展开每个零件的配比后把所有原料数量求和，输出单个浮点数即所需原料的总件数，连线到下游数值端口即可参与效果/检查计算）。
              </div>
              <div v-else-if="selectedNode.data.type === 'productList'" class="ge-tip">
                ...
                <b>所需的科技节点</b>（按比赛查询清单中每个产品的「科技需求(TechNode)」，收集这些科技节点名称，去重后输出字符串数组，连线到下游列表/字典端口即可直接拿到该批产品所需的全部科技节点，无需手动查表）、
                <b>产品总件数</b>（直接把清单中各产品数量相加，连线到下游数值端口即可参与效果/检查计算，无需查表）<b>与</b>
                <b>所需零件总数量</b>（按比赛展开每个产品的零件配比后把所有零件数量求和，输出单个浮点数即所需零件的总件数，连线到下游数值端口即可参与效果/检查计算）。
              </div>
              ...
              <div v-else-if="selectedNode.data.type === 'nodeRoute'" class="ge-tip">
                ...
                <br />
                本输入源有五个输出端点：<b>输出</b>（原始节点 id 数组，可接入列表/公式）、
                <b>路程</b>（按比赛查询地图相邻节点最短路径距离之和，连线到下游数值端口即可参与效果/检查计算，无需再绕到数据源节点的「节点列表」端口）、
                <b>存在的路径类型</b>（按比赛查询与任一路点相连的边，取这些边所用路径类型的名称组成列表，可接入下游列表/字典端口）、
                <b>起始节点名</b>（节点列表第一个节点的名称，字符串）<b>与</b>
                <b>终止节点名</b>（节点列表最后一个节点的名称，字符串）。
              </div>
