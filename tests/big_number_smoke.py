...
5. company_fields.timer 的数值解析函数

审计 X-17 修复点
----------------
① 改前本文件在**模块顶层**执行 `sys.path` 注入、`django.setup()`，并在结尾 `sys.exit()`；
   被 pytest 收集时，collection 阶段就会导入模块并跑完整套逻辑再抛 `SystemExit`。
   现在全部收进 `main()`，只有 `__main__` 才执行；引导统一走 `tests/_bootstrap.py`。
② 第 6 组此前 `from apps.company_fields.timer import _to_num` 引用了**不存在的函数名**
   （实际是 `_parse_number_raw`），ImportError 被转成一行 `[SKIP]`，于是该组从未真正执行，
   而脚本仍报告 `PASS=20 FAIL=0`（审计 M-2 / E-6）。现在改用真实函数名，并且
   **找不到就 FAIL 而不是 SKIP** —— 静默降级等于没测。
...
def main() -> int:
    ...
    print("== 6. company_fields.timer 数值解析（审计 M-2：不得静默 SKIP） ==")
    try:
        from apps.company_fields.timer import _parse_number_raw  # type: ignore[attr-defined]
    except ImportError as e:
        check("timer._parse_number_raw 可导入", False, f"ImportError: {e}")
    else:
        check("timer._parse_number_raw 大数", _parse_number_raw(str(BIG)) == BIG,
              repr(_parse_number_raw(str(BIG))))
        check("timer._parse_number_raw 小数", _parse_number_raw("123.45") == Decimal("123.45"))
